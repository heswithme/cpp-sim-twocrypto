// Unified Stableswap math (templated, parity with Vyper StableswapMath)
#pragma once
...
struct MathOps<uint256> : MathOpsCommon<uint256> {
    ...
    static T wad_exp(const int256& x) {
        static const int256 MIN_EXP_INPUT("-41446531673892822313"); if (x <= MIN_EXP_INPUT) return 0;
        static const int256 MAX_EXP_INPUT("135305999368893231589"); if (x >= MAX_EXP_INPUT) throw std::overflow_error("math: wad_exp overflow");
        ...
        int256 y = (x_scaled + int256("1346386616545796478920950773328")) * x_scaled; y = (y >> 96) + int256("57155421227552351082224309758442");
        int256 p = y + x_scaled - int256("94201549194550492254356042504812"); p = p * y; p = (p >> 96) + int256("28719021644029726153956944680412240"); p = p * x_scaled; p = p + (int256("4385272521454847904659076985693276") << 96);
        int256 q = x_scaled - int256("2855989394907223263936484059900"); q = q * x_scaled; q = (q >> 96) + int256("50020603652535783019961831881945"); q = q * x_scaled; q = (q >> 96) - int256("533845033583426703283633433725380"); q = q * x_scaled; q = (q >> 96) + int256("3604857256930695427073651918091429"); q = q * x_scaled; q = (q >> 96) - int256("14423608567350463180887372962807573"); q = q * x_scaled; q = (q >> 96) + int256("26449188498355588339934803723976023");
        int256 r = p / q;
        ...
        uint256 result; if (shift_amount > 0) result = (r_unsigned * SCALE_FACTOR) >> shift_amount; else if (shift_amount < 0) result = (r_unsigned * SCALE_FACTOR) << (-shift_amount); else result = r_unsigned * SCALE_FACTOR; return result;
    }
};
