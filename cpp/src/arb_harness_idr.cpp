...
static std::vector<Event> load_events(const std::string& path, size_t max_events = 0) {
    ...
    for (size_t i = 0; i < limit; ++i) {
        ...
        if (e.is_array()) {
            ...
            } else if (auto* v = o.if_contains("timestamp")) { // Added for IDR data
                if (v->is_uint64()) ts = v->as_uint64();
                else if (v->is_int64()) ts = static_cast<uint64_t>(v->as_int64());
                else if (v->is_double()) ts = static_cast<uint64_t>(v->as_double());
            }
            ...
            else if (auto* v = o.if_contains("price_open")) price = to_d(*v); // Added for IDR data
            else if (auto* v = o.if_contains("price_vwap")) price = to_d(*v); // Added for IDR data
            else if (auto* v = o.if_contains("price_hight")) price = to_d(*v); // Added for IDR data
            else if (auto* v = o.if_contains("price_low")) price = to_d(*v); // Added for IDR data
